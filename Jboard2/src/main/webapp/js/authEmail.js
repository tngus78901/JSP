				...
				success: function(data) {
						...
						if(data.result > 0) {
							...
							if(data.status > 0) {
									...
									$('.resultEmailForId').css('color','red').text('인증코드 전송이 실패 했습니다. 잠시후 다시 시도하십시오.');
							}
								...
								if(data.status >0 ){
									...
									$('.resultEmail').css('color','red').text('인증코드 전송이 실패 했습니다. 잠시후 다시 시도하십시오.');
				...
				$.ajax({
						...
						success: function(data) {
								...
								if(data.result > 0) {
										...
										$('.resultEmailForId').css('color','green').text('')
										isEmailOk = true;	
								}else {
										$('.resultEmail').css('color','red').text('이메일 인증이 실패 했습니다. 다시 시도합시시요.');
										$('.resultEmailForId').css('color','red').text()
										isEmailOk = false;
								}
						}
